                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (typeof data === 'string') {
                                commentResult = jQuery.parseJSON(data);
                            } else {
                                commentResult = data;
                            }
                            ...
                            if (commentResult.Data && commentResult.Data.Comments.$values) {
                                const articlesArray = [];
                                $.each(commentResult.Data.Comments.$values, function (index, comment) {
                                    // Mevcut getJsonNetObject fonksiyonunuzu kullanın
                                    ...
                                    const newTableRow = dataTable.row.add([
                                        newComment.Id,
                                        newArticle ? newArticle.Title : 'Başlık Yok',
                                        newComment.Text ? (newComment.Text.length > 75 ? newComment.Text.substring(0, 75) : newComment.Text) : '',
                                ...
                                toastr.error(`${commentResult.Message || "Bilinmeyen hata"}`, 'İşlem Başarısız!');
                                $('.spinner-border').hide();
