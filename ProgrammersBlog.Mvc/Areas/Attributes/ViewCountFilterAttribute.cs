...
namespace ProgrammersBlog.Mvc.Attributes
{
    ...
    public class ViewCountFilterAttribute:Attribute,IAsyncActionFilter
    {
        ...
        {
            // Önce articleId parametresinin var olup olmadığını kontrol et
            if (context.ActionArguments.ContainsKey("articleId") && context.ActionArguments["articleId"] != null)
            {
                ...
                if (string.IsNullOrEmpty(articleValue))
                {
                    ...
                    await articleService.IncreaseViewCountAsync(Guid.Parse(articleId.ToString()));
                }
            }
            
            // Her durumda işlemi devam ettir
            await next();
        }
        /// <summary>  
        /// set the cookie  
